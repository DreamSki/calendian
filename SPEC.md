...
| REQ-NOTE-004 | THE SYSTEM SHALL handle missing or renamed notes safely. | P0 | v0.5 | Implemented — `getAssociatedNotes()` filters out nonexistent files; `resolveNotePath()` search-by-title for renamed notes; index rebuilt every 60s via setInterval + on window focus |
...
| REQ-NOTE-007 | THE SYSTEM SHOULD insert associated event/reminder links into daily notes. | P1 | v0.5 | Implemented — `renderCalendianInline()` renders `cal:ev:ID`/`cal:rem:ID` inline refs as styled mini-table in any note; `renderCalendianBlock()` renders `` ```calendian `` code blocks with events/reminders for target date; user places refs manually (plugin never auto-modifies notes) |
